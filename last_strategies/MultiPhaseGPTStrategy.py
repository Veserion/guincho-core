...
class MultiPhaseGPTStrategy(IStrategy):
    timeframe = '1h'
    stoploss = -0.05

    # Параметры для определения фазы
    sma_length = IntParameter(50, 250, default=200, space='buy')
    adx_threshold = IntParameter(15, 60, default=20, space='buy')
    ...
    bollinger_period = IntParameter(10, 50, default=20, space="buy")
    bollinger_dev = DecimalParameter(1.5, 3.0, default=2.0, space="buy")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Bollinger Bands
        bollinger = ta.BBANDS(dataframe, timeperiod=self.bollinger_period.value, nbdevup=self.bollinger_dev.value,
                              nbdevdn=self.bollinger_dev.value)
    ...
    def calculate_range_condition(self, dataframe: DataFrame) -> pd.Series:
        return (
                ...
                (dataframe['rsi'] < 30) &
                (dataframe['atr'] < self.atr_multiplier.value * dataframe['atr'].rolling(10).mean())
        )
    ...
    def calculate_strong_trend_condition(self, dataframe: DataFrame, base_condition: pd.Series) -> pd.Series:
        ...
        if self.use_ema_fast_slow.value:
            condition &= (dataframe['ema_fast'] > dataframe['ema_slow'])

        if self.use_rsi.value:
            condition &= (dataframe['rsi'] > self.rsi_low.value) & (dataframe['rsi'] < self.rsi_high.value)
