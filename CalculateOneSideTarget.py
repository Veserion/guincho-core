...
class CalculateOneSideTarget(IStrategy):
    timeframe = '1h'
    stoploss = -0.05
    ...
    lookahead_period_sell = IntParameter(1, 20, default=10, space='sell')
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        price_change = (dataframe['close'].shift(-self.lookahead_period.value) - dataframe['close']) / dataframe['close']
        price_change_sell = (dataframe['close'].shift(-self.lookahead_period_sell.value) - dataframe['close']) / dataframe['close']
