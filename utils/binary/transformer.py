...
transformer_model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10, batch_size=32, verbose=1,
...
# Для XGBoost нам нужно преобразовать данные в плоский формат
X_train_flat = X_train.reshape((X_train.shape[0], -1))  # Преобразуем из (samples, time_steps, features) в (samples, time_steps*features)
...
xgb_model = XGBClassifier(
    ...
    early_stopping_rounds=10  # Перенесено из fit() в инициализацию
)

# Определение валидационного набора
...
# Преобразование вероятностей в бинарные метки для XGBoost
y_pred_xgb_bin = (y_pred_xgb >= 0.5).astype(int)

# Преобразование комбинированных предсказаний в бинарные метки
y_pred_combined_bin = (y_pred_combined >= 0.5).astype(int)

# Теперь классификационный отчет
print("Transformer model classification report:")
print(classification_report(y_test, y_pred_transformer_bin))

cm_transformer = confusion_matrix(y_test, y_pred_transformer_bin)
sns.heatmap(cm_transformer, annot=True, fmt='d', cmap='Blues', xticklabels=[0, 1], yticklabels=[0, 1])
...
y_pred_combined_bin = y_pred_combined_bin.flatten()
print(classification_report(y_test, y_pred_combined_bin))

cm_combined = confusion_matrix(y_test, y_pred_combined_bin)
sns.heatmap(cm_combined, annot=True, fmt='d', cmap='Blues', xticklabels=[0, 1], yticklabels=[0, 1])
plt.xlabel("Predicted")
plt.ylabel("Actual")
