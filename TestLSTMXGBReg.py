...
from ml_regression.config import TIME_STEPS, LSTM_MODEL_PATH, XGB_MODEL_PATH
from ml_regression.utils import create_predict_sequences, flatten_sequences
...
class TestLSTMXGBReg(IStrategy):
    timeframe = '30m'
    stoploss = -0.07
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        lstm_predictions = self.lstm_model.predict(sequences, verbose=0).flatten()
        xgb_predictions = self.xgb_model.predict(flatten_sequences(sequences)).flatten()
        # Добавляем NaN в начале, чтобы длина совпала с dataframe
        nan_padding = [np.nan] * TIME_STEPS
        predictions = np.concatenate((nan_padding, (lstm_predictions + xgb_predictions)/2))
